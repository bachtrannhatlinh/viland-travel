...
export const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: user, error } = await supabaseAuthService.registerUser({
      firstName,
      lastName,
      email,
      password,
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!secret) {
      ...
      res.status(500).json({
        success: false,
        error: { message: "JWT secret is not configured on the server." }
      });
      return;
    }
    ...
    try {
      ...
      res.status(500).json({
        success: false,
        error: { message: "Error signing JWT token.", details: signError instanceof Error ? signError.message : signError }
      });
      return;
    }
...
export const getProfile = async (
  ...
  try {
    ...
    const { data: user, error } = await supabaseAuthService.getUserProfile(
      userId
    );
    if (error || !user) {
      res.status(404).json({
...
export const forgotPassword = async (
  ...
  try {
    ...
    const { data: user, error } = await supabaseAuthService.getUserByEmail(
      email
    );
    if (error || !user) {
...
export const updateProfile = async (
  ...
  try {
    ...
    const result = await supabaseAuthService.updateUserProfile(userId, {
      firstName,
      lastName,
      phone,
    });
