...
export default function FlightCard({ flight, onSelect }: FlightCardProps) {
  const getLowestPrice = () => {
    ...
    const prices = Object.values(flight.pricing)
      .map((p) => p?.price || 0)
      .filter((p) => p > 0);
  ...
  const hasDiscount = () => {
    ...
    return Object.values(flight.pricing).some(
      (p) => p?.originalPrice && p?.originalPrice > (p?.price || 0)
    );
  };
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {parseDateWithTimezone(flight.departure_date)?.toLocaleTimeString(
                "vi-VN",
                {
                  hour: "2-digit",
                  minute: "2-digit",
            ...
            <div className="text-sm text-gray-500">
              {parseDateWithTimezone(flight.departure_date)?.toLocaleDateString(
                "vi-VN",
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-gray-900">
              {parseDateWithTimezone(flight.arrival_date)?.toLocaleTimeString(
                "vi-VN",
                {
                  hour: "2-digit",
                  minute: "2-digit",
            ...
            <div className="text-sm text-gray-500">
              {parseDateWithTimezone(flight.arrival_date)?.toLocaleDateString(
                "vi-VN",
                { day: "2-digit", month: "2-digit", year: "numeric" }
              ) ?? ""}
            </div>
        ...
        {flight.amenities && (
          <div className="flex items-center space-x-4 mb-4 text-sm text-gray-600">
            {(Array.isArray(flight.amenities)
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200">
          <div className="flex space-x-2">
            {flight.pricing?.economy?.available &&
              flight.pricing.economy.available > 0 && (
                <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">
                  Phổ thông: {flight.pricing.economy.available} chỗ
                </span>
              )}
            {flight.pricing?.business?.available &&
              flight.pricing.business.available > 0 && (
          ...
          <div className="flex items-center space-x-3">
            ...
            <Button onClick={() => onSelect(flight)}>Chọn chuyến bay</Button>
          </div>
        </div>
      </div>
    </div>
